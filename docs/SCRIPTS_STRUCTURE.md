...
- `monitor-device.js` - Мониторинг конкретного устройства через WebSocket
- `monitor-device-lazy.js` - Мониторинг с ленивым кэшем на БД
...
- `test-get-state.js` - Тест получения состояния через WebSocket
- `test-ws-server.js` - Тест WebSocket сервера

## 📦 scripts/archive/ - Архив временных скриптов

### debug/ - Отладочные скрипты
- `daemon_load_fixed.js` - Исправленная версия загрузки демона
- `daemon_load_with_logging.js` - Загрузка демона с логированием
- `event-log-from-pi.js` - Логирование событий с Pi
- `event-logger-clean.js` - Очистка логгера событий
- `event-logger-current.js` - Текущая версия логгера событий
- `event-logger-from-pi.js` - Логгер событий с Pi
- `fix-syntax.js` - Исправление синтаксиса

### analysis/ - Аналитические скрипты (в archive/)
