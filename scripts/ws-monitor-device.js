...
// Получаем ID устройства/устройств из аргументов командной строки (через запятую)
const TARGET_DEVICE_ID = process.argv[2];
const TARGET_DEVICE_IDS = (TARGET_DEVICE_ID || '')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);

if (TARGET_DEVICE_IDS.length === 0) {
  console.error('❌ Ошибка: Не указан ID устройства');
  console.error('');
  console.error('Использование:');
  console.error('  node scripts/ws-monitor-device.js <device-id|id1,id2,...>');
  console.error('');
...
console.log(`   ID устройства(ов): ${TARGET_DEVICE_IDS.join(', ')}`);
console.log('');
...
ws.on('message', (data) => {
  ...
  try {
    ...
    // Зачем: Показываем ВСЕ сообщения, связанные с искомыми устройствами
    const isDirectMatch = TARGET_DEVICE_IDS.includes(messageId);
    const containsId = TARGET_DEVICE_IDS.some(id => messageStr.includes(id));
