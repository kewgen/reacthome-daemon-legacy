...
const add = (id, oldState, newState, context, changedPayload = null) => {
  ...
  // Зачем: проверяем, что executed или last_execution присутствуют в payload (изменённых полях)
  const hasExecutedInPayload = changedPayload && ('executed' in changedPayload);
  const hasLastExecutionInPayload = changedPayload && ('last_execution' in changedPayload);
  const hasExecutedInNewState = newState && newState.executed !== undefined;
  const hasLastExecutionInNewState = newState && newState.last_execution !== undefined;
  
  if (hasExecutedInPayload || hasLastExecutionInPayload || hasExecutedInNewState || hasLastExecutionInNewState) {
    // Определяем параметр: приоритет executed над last_execution
    const param = (hasExecutedInPayload || hasExecutedInNewState) ? 'executed' : 'last_execution';
    ...
    // Зачем: получаем новое значение из changedPayload (если есть) или из newState
    const newValue = hasExecutedInPayload ? changedPayload.executed : 
                     hasLastExecutionInPayload ? changedPayload.last_execution :
                     hasExecutedInNewState ? newState.executed : 
                     hasLastExecutionInNewState ? newState.last_execution : null;
    ...
    // Зачем: получаем полное состояние устройства для name, code, title (newState может содержать только изменённые поля)
    const deviceState = state.get(id) || newState;
    
    // Для скриптов получаем site из родительской локации
    // Зачем: для сценариев ищем site через различные связи (site, project, parent)
    let siteName = getSiteName(id, 10, new Set(), false);
    
    // Если не нашли через id, пробуем через parent
    if (!siteName && deviceState.parent) {
      siteName = getSiteName(deviceState.parent, 10, new Set(), false);
    }
    
    // Если не нашли через parent, пробуем через site (если есть)
    if (!siteName && deviceState.site) {
      const siteId = Array.isArray(deviceState.site) ? deviceState.site[0] : deviceState.site;
      if (siteId && typeof siteId === 'string') {
        siteName = getSiteName(siteId, 10, new Set(), false);
      }
    }
    
    // Если не нашли через site, пробуем через project
    if (!siteName && deviceState.project) {
      if (typeof deviceState.project === 'string') {
        siteName = getSiteName(deviceState.project, 10, new Set(), false);
      }
    }
    const event = {
      timestamp: Date.now(),
      ...
      device: {
        type: null,
        human: getHumanName(deviceState),
        // Зачем: сохраняем оригинальные значения полей в OpenSearch, включая пустые строки
        name: deviceState.name !== undefined ? deviceState.name : null,
        code: deviceState.code !== undefined ? deviceState.code : null,
        title: deviceState.title !== undefined ? deviceState.title : null
      },
      param,
      old: roundedOldValue,
      new: roundedNewValue,
      trigger: {
        type: context.type || 'script', // Используем тип из контекста или 'script' по умолчанию
        ref: context.ref || id, // ref из контекста или id сценария
...
const init = async () => {
  ...
  // Периодическая очистка старых логов и кэшей (каждые 10 минут)
  setInterval(async () => {
    await cleanupOldLogs();
    const dirSizeMB = await checkLogDirSize();
    if (dirSizeMB > MAX_LOG_DIR_SIZE_MB) {
    ...
    // Очистка siteNameCache (зачем: удаление устаревших записей для предотвращения утечки памяти)
    const now = Date.now();
    let cleanedSiteNames = 0;
    for (const [key, cached] of siteNameCache.entries()) {
