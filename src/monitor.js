...
class TerminalKitStatusDisplay {
  constructor(devices, sites, locationName = 'Локация') {
    ...
    // Зачем: глобальный индекс обратных привязок для быстрого поиска устройств по актуатору
    // Структура: Map<actuatorMac, Map<channelId, devices[]>>
    // Пример: bindingsIndex.get('54:10:ec:19:ce:8b').get('do/1') → [{ deviceId, deviceTitle, ... }]
    // Алгоритм согласно ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
    // Документация: docs/06-devices/actuators/ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
    this.bindingsIndex = null;
  ...
  updateDevices(devices, sites, locationName) {
    ...
    // Зачем: строим индекс обратных привязок после загрузки всех устройств
    // Это позволяет быстро находить устройства, привязанные к каналам актуатора
    // Алгоритм согласно ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
    this.buildBindingsIndex();
  ...
  getActuatorChannels(actuatorId, deviceType) {
    ...
    // Зачем: получаем обратные привязки из глобального индекса для быстрого поиска
    // Это позволяет находить устройства, которые ссылаются на этот актуатор
    const reverseBindings = this.bindingsIndex?.get(actuatorId) || new Map();
    ...
    for (let i = 1; i <= doCount; i++) {
      ...
      if (channelState && channelState.bind) {
        ...
        // Зачем: отладочное логирование для диагностики резолва прямых привязок
        // Помогает понять, почему устройства не найдены (например, R2 вытяжки)
        if (!linkedDevice && channelState.bind && channelState.bind.length > 10) {
          console.log(`[DEBUG] ${channelId}: устройство ${channelState.bind.substring(0, 8)}... не найдено в allDevices (всего ${this.allDevices.length})`);
        }
      ...
      // Обратная привязка (Тип B): устройство → канал актуатора
      // Зачем: находим устройства, которые ссылаются на этот канал в своём bind
      // Алгоритм согласно ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md, шаг 4.1 и 4.2
      // Документация: docs/06-devices/actuators/ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
      const linkedDevices = reverseBindings.get(`do/${i}`) || [];
      ...
      channels.push({ 
        ...
        channelIndex: i, 
        channelState, 
        linkedDevice,      // Одно устройство (Тип A: канал → устройство)
        linkedDevices      // Массив устройств (Тип B: устройство → канал)
      });
    }
    ...
    for (let i = 1; i <= dimCount; i++) {
      ...
      if (channelState && channelState.bind) {
        ...
        // Зачем: отладочное логирование для диагностики резолва
        if (!linkedDevice && channelState.bind && channelState.bind.length > 10) {
          console.log(`[DEBUG] ${channelId}: устройство ${channelState.bind.substring(0, 8)}... не найдено`);
        }
      ...
      // Обратная привязка (Тип B): устройство → канал актуатора
      const linkedDevices = reverseBindings.get(`dim/${i}`) || [];
    ...
    for (let i = 1; i <= aoCount; i++) {
      ...
      // Обратная привязка (Тип B): устройство → канал актуатора
      const linkedDevices = reverseBindings.get(`ao/${i}`) || [];
    ...
    // Зачем: добавляем группы каналов с обратными привязками (только если есть привязки)
    // Группы используются для управления несколькими каналами одновременно (шторы, сложное освещение)
    // Формат привязки: actuatorMac/group/N (например, 54:10:ec:19:ce:8b/group/3)
    // Алгоритм согласно ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md, примеры групп
    // Документация: docs/06-devices/actuators/ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
    for (let i = 1; i <= 10; i++) {
      const linkedDevices = reverseBindings.get(`group/${i}`) || [];
      if (linkedDevices.length > 0) {
        channels.push({
          channelId: `${actuatorId}/group/${i}`,
          channelType: 'group',
          channelIndex: i,
          channelState: null,  // Группы не имеют собственного состояния
          linkedDevice: null,
          linkedDevices
        });
      }
  ...
  // Строит глобальный индекс обратных привязок: актуатор → устройства
  // Зачем: поиск устройств, которые ссылаются на актуатор в своём bind (обратный поиск)
  // Вызывается один раз при загрузке данных для оптимизации производительности
  // Алгоритм согласно ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md, шаг 4
  // Документация: docs/06-devices/actuators/ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
  buildBindingsIndex() {
    const index = new Map(); // actuatorMac -> Map<channelId, devices[]>
    let bindingsCount = 0;
    ...
    this.allDevices.forEach(device => {
      ...
      // 1. Проверяем прямую привязку device.bind (шаг 4.1)
      // Формат: <MAC>/<kind>/<index> (например, "54:10:ec:19:ce:8b/do/1" или "54:10:ec:19:ce:8b/group/3")
      const bind = deviceState.bind;
      if (bind && typeof bind === 'string' && bind.includes('/')) {
        const parts = bind.split('/');
        if (parts.length >= 3) {
          const [mac, kind, idx] = parts;
          // Проверяем что это MAC-адрес актуатора (содержит ':')
          if (mac && mac.includes(':')) {
            if (!index.has(mac)) {
              index.set(mac, new Map());
            }
            const channelId = `${kind}/${idx}`;
            if (!index.get(mac).has(channelId)) {
              index.get(mac).set(channelId, []);
            }
            index.get(mac).get(channelId).push({
              deviceId: device.id,
              deviceType: device.type,
              deviceTitle: device.title || device.code || device.name || device.id,
      ...
      // 2. Проверяем привязки в каналах device.channels[].bind (шаг 4.2)
      // Формат: channels[chIndex].bind = "<MAC>/<kind>/<index>"
      if (deviceState.channels && typeof deviceState.channels === 'object') {
        Object.entries(deviceState.channels).forEach(([chIdx, channel]) => {
          const chBind = channel.bind;
          if (chBind && typeof chBind === 'string' && chBind.includes('/')) {
            const parts = chBind.split('/');
            if (parts.length >= 3) {
              const [mac, kind, idx] = parts;
              if (mac && mac.includes(':')) {
                if (!index.has(mac)) {
    ...
    console.log(`[INFO] Построен индекс привязок: ${index.size} актуаторов, ${bindingsCount} привязок (только обратные)`);
    
    // Зачем: подсчитываем прямые привязки (канал → устройство) для статистики
    // Это поможет понять, сколько каналов имеют прямые привязки vs обратные
    let directBindingsCount = 0;
    this.allDevices.forEach(device => {
      if (device.category === 'Актуатор' && typeof device.type === 'number') {
        ...
        if (channelConfig) {
          ...
          // Проверяем DO каналы
          if (channelTypes.includes('do')) {
            for (let i = 1; i <= channelCount; i++) {
              const channelId = `${device.id}/do/${i}`;
              const channelState = this.deviceStates.get(channelId)?.state;
              if (channelState?.bind) {
                directBindingsCount++;
              }
            }
          }
          
          // Проверяем DIM каналы
          if (channelTypes.includes('dim')) {
            for (let i = 1; i <= channelCount; i++) {
              const channelId = `${device.id}/dim/${i}`;
              const channelState = this.deviceStates.get(channelId)?.state;
              if (channelState?.bind) {
    ...
    console.log(`[INFO] Найдено прямых привязок (канал→устройство): ${directBindingsCount}`);
  }
  ...
  getDeviceInfoText(device) {
    ...
    if (isActuator) {
      ...
      if (channels.length > 0) {
        // Зачем: подсчитываем привязки для информативного заголовка
        const totalBindings = channels.reduce((sum, ch) => {
          const directBind = ch.linkedDevice ? 1 : 0;
          const reverseBinds = (ch.linkedDevices && ch.linkedDevices.length) || 0;
          return sum + directBind + reverseBinds;
        }, 0);
        
        info.push('');
        info.push(`Actuator channels: (${totalBindings} привязок найдено)`);
        ...
        Object.keys(channelsByType).sort().forEach(channelType => {
          ...
          // Зачем: подсчитываем привязки для каждого типа каналов
          const typeBindings = typeChannels.reduce((sum, ch) => {
            const directBind = ch.linkedDevice ? 1 : 0;
            const reverseBinds = (ch.linkedDevices && ch.linkedDevices.length) || 0;
            return sum + directBind + reverseBinds;
          }, 0);
          ...
          const typeName = channelType === 'do' ? 'Реле (DO)' 
                        ...
                        : channelType === 'group' ? 'Группы'
                        : channelType;
          
          info.push(`  ${typeName} каналы: (${typeBindings} привязок)`);
          ...
          typeChannels.forEach(channel => {
            ...
            } else if (channel.linkedDevices && channel.linkedDevices.length > 0) {
              // Зачем: отображаем обратные привязки (устройства, которые ссылаются на этот канал)
              // Алгоритм согласно ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md, шаг 5
              // Документация: docs/06-devices/actuators/ACTUATOR_BINDING_RESOLUTION_ALGORITHM.md
              channel.linkedDevices.forEach((linkedDev, idx) => {
                const displayName = linkedDev.deviceTitle || linkedDev.deviceId.substring(0, 8) + '...';
                const deviceType = linkedDev.deviceType;
                const deviceSite = linkedDev.deviceSite || '';
                
                const prefix = idx === 0 ? channelLine : `    ${' '.repeat(channelTypeName.length + 3)}`;
                let line = `${prefix} → ${displayName} (${deviceType})`;
                if (deviceSite) {
                  line += ` / ${deviceSite}`;
                }
                info.push(line);
