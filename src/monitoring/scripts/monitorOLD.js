 ...
 * 
 * ПРИМЕЧАНИЕ: На Raspberry Pi зависимости должны быть уже установлены в проекте.
 * Если скрипт копируется отдельно, убедитесь что зависимости установлены:
 *   cd /path/to/project-directory
 *   npm install terminal-kit ws
 * 
 * ИСПОЛЬЗОВАНИЕ:
 * ==============
 * На Raspberry Pi (скрипт должен быть в директории проекта):
 *   cd /path/to/reacthome-daemon  # или любая другая директория проекта
 *   node src/monitor.js            # или node monitor.js если скрипт в корне
 * 
 * С указанием URI WebSocket (если сервер на другом хосте/порту):
 ...
 *   export REACTHOME_WS_URI=ws://localhost:3000  # или IP адрес малинки с сервером
 * 
 * Запуск скрипта на Raspberry Pi:
 *   cd /path/to/reacthome-daemon  # перейти в директорию проекта
 *   node src/monitor.js            # запустить скрипт
 *   
 *   # Или с указанием URI WebSocket (если сервер на другом хосте)
 *   REACTHOME_WS_URI=ws://192.168.1.100:3000 node src/monitor.js
 *   
 * 
 ...
 *   Ошибка: Cannot find module 'terminal-kit' или 'ws'
 *   Решение:
 *     cd /path/to/reacthome-daemon  # замените на реальный путь
 *     npm install terminal-kit ws
 * 
 ...
 *        cd /path/to/reacthome-daemon
 *        export REACTHOME_WS_URI=ws://<IP_АДРЕС_СЕРВЕРА>:3000
 *        node src/monitor.js
 *     3. Проверить доступность порта:
 *        telnet <IP_АДРЕС_СЕРВЕРА> 3000  # или localhost если сервер локально
 * 
...
// Зачем: URI WebSocket по умолчанию - localhost (скрипт запускается на той же малинке, где сервер)
// Можно переопределить через переменную окружения REACTHOME_WS_URI для работы с удалённым сервером
...
function loadDevicesAndSitesViaWebSocket(wsUri) {
  return new Promise((resolve, reject) => {
    const devices = [];
    const sites = [];
    const siteMap = new Map(); // Зачем: Карта помещений для резолвинга UUID в названия
    ...
    function processDevicesAndSites() {
        ...
        if (payload.site) {
          ...
          siteIds.forEach(siteId => {
            if (typeof siteId === 'string' && !siteMap.has(siteId)) {
              // Зачем: Если помещение не найдено как отдельное устройство, используем ID как имя
      ...
      // Зачем: Сохраняем состояния каналов актуаторов из deviceDataMap
      // Это нужно для того, чтобы привязки каналов были доступны в getActuatorChannels
      const channelStates = new Map();
      deviceDataMap.forEach((payload, deviceId) => {
        // Зачем: Сохраняем только каналы (ID содержат '/')
        if (deviceId.includes('/')) {
          channelStates.set(deviceId, payload);
        }
      });
      ...
      resolve({
        ...
        channelStates: channelStates // Зачем: Передаем состояния каналов для сохранения в deviceStates
      });
    }
  });
}
...
class TerminalKitStatusDisplay {
  ...
  getActuatorChannels(actuatorId, deviceType) {
    ...
    for (let i = 1; i <= doCount; i++) {
      ...
      if (channelState && channelState.bind) {
        ...
        // Зачем: Если устройство не найдено, запрашиваем через WebSocket
        if (!linkedDevice && typeof channelState.bind === 'string') {
  ...
  requestMissingDevice(deviceId) {
    ...
    // Зачем: Запрашиваем устройство через WebSocket
  ...
  addMissingDevice(deviceId, payload) {
    ...
    // Зачем: Обрабатываем случаи, когда payload отсутствует или пустой
    if (!payload) {
      console.log(`[DEBUG] Устройство ${deviceId} не имеет payload, создаем минимальное устройство`);
      payload = {
        type: 0x00,
    ...
    // Зачем: Если тип не указан, используем неизвестный тип
    if (payload.type === undefined || payload.type === null) {
      console.log(`[DEBUG] Устройство ${deviceId} не имеет типа, используем неизвестный тип`);
      payload.type = 0x00;
    }
    ...
    // Зачем: Обрабатываем устройства с неизвестным типом (0x00) или без типа
    if (!device && (deviceType === 0x00 || !deviceType)) {
      // Зачем: Создаем минимальное устройство для отображения
      const deviceName = payload.name || payload.title || payload.code || `Устройство ${deviceId.substring(0, 8)}...`;
      let siteId = payload.site;
      let siteName = null;
...
async function main() {
  try {
    ...
    const { devices, sites, channelStates } = await loadDevicesAndSitesViaWebSocket(WS_URI);
    ...
    // Зачем: Сохраняем состояния каналов актуаторов в deviceStates
    // Это нужно для того, чтобы привязки каналов были доступны в getActuatorChannels
    if (channelStates && channelStates.size > 0) {
      console.log(`Загружено ${channelStates.size} состояний каналов актуаторов`);
      channelStates.forEach((payload, channelId) => {
        display.setDeviceState(channelId, payload);
      });
    }
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (isActionSet && message.id) {
          ...
          const payload = message.payload || {}; // Зачем: Обрабатываем случаи, когда payload отсутствует или пустой
          ...
          if (isMissingDevice && !deviceExists) {
            if (payload && payload.type) {
              // Зачем: Устройство с данными, добавляем его
              console.log(`[DEBUG] Добавляем запрошенное устройство: ${deviceId}, тип: ${payload.type}`);
              display.addMissingDevice(deviceId, payload);
            } else {
              // Зачем: Payload пустой, создаем минимальное устройство для отображения
              console.log(`[DEBUG] Устройство ${deviceId} найдено, но payload пустой, создаем минимальное устройство`);
              display.addMissingDevice(deviceId, {
                type: 0x00, // Зачем: Неизвестный тип
                name: `Устройство ${deviceId.substring(0, 8)}...`,
                id: deviceId
              });
            }
          ...
          if (!deviceExists && payload && payload.type) {
          ...
          // Зачем: Обрабатываем случаи, когда payload отсутствует, но устройство упоминается в bind
          if (!deviceExists && (!payload || !payload.type)) {
            const isReferencedInBind = Array.from(display.allDevices).some(device => {
              if (device.category === 'Актуатор' && typeof device.type === 'number') {
                const channels = display.getActuatorChannels(device.id, device.type);
            ...
            if (isReferencedInBind && !display.requestedMissingDevices.has(deviceId)) {
              // Зачем: Устройство упоминается в bind, но payload пустой, создаем минимальное устройство
              console.log(`[DEBUG] Устройство ${deviceId} упоминается в bind, но payload пустой, создаем минимальное устройство`);
              display.requestedMissingDevices.add(deviceId);
              display.addMissingDevice(deviceId, {
                type: 0x00,
                name: `Устройство ${deviceId.substring(0, 8)}...`,
                id: deviceId
          ...
          // Зачем: Обрабатываем как устройства, так и каналы актуаторов (ID содержат '/')
