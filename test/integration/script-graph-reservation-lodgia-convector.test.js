...
/**
 * Регресс: SOURCE приходит не по base value, а как DI-count по /di/4 (onHoldCount).
 * Зачем: в бою долгий тап S4 часто выражается именно как onHoldCount, а триггеры лежат в DI объекте канала.
 */
test('script-graph reservation: DI/4 onHoldCount -> nested script -> Convector Lodgia in one trace', { timeout: 30000 }, async (t) => {
  if (process.env.RUN_INTEGRATION !== '1') {
    t.skip('RUN_INTEGRATION!=1');
    return;
  }
  ...
  const baseS4Id = '90:89:d3:f1:2d:0f';
  const di1Id = `${baseS4Id}/di/1`;
  const di4Id = `${baseS4Id}/di/4`;
  ...
  const t0 = 1766519620000;
  const tDown = t0 + 1000;
  const tNestedExecuted = tDown + 250; // имитируем быстрый эффект после onHoldCount
  const tConvectorOn = tNestedExecuted + 2;

  const initStateMap = new Map([
    [baseS4Id, { type: 37, code: 'S4 Лоджия', title: 'S4 Лоджия', timestamp: t0, value: 0 }],
    // di/1 может быть пустой/не содержать onHold — важно, чтобы работал diSelf (di/4)
    [di1Id, { value: 0, timestamp: t0, onClick: [], onClick2: [] }],
    [di4Id, { value: 0, timestamp: t0, onHold: [holdScriptId] }],
  ...
  const baseTmp = path.join(process.cwd(), 'var', 'tmp');
  fs.mkdirSync(baseTmp, { recursive: true });
  const tmpRoot = fs.mkdtempSync(path.join(baseTmp, 'script-graph-reservation-di4-'));
  ...
  try {
    ...
    // SOURCE: onHoldCount по DI/4
    ws.send(JSON.stringify({ type: 'ACTION_SET', id: di4Id, payload: { onHoldCount: 1, timestamp: tDown } }));
    await sleep(50);
    ...
    const src = allEvents.find((e) => e && e.id === di4Id && e.param === 'onHoldCount' && e.timestamp === tDown);
    assert.ok(src && typeof src.trace_id === 'string', 'SOURCE DI/4 onHoldCount должен иметь trace_id');

    const nested = allEvents.find((e) => e && e.id === nestedScriptId && e.param === 'executed' && e.new === true);
    assert.ok(nested && typeof nested.trace_id === 'string', 'nested SCRIPT executed должен иметь trace_id');
    assert.equal(nested.trace_id, src.trace_id, 'nested SCRIPT executed обязан наследовать trace_id источника DI/4');

    const conv = allEvents.find((e) => e && e.id === convectorId && e.param === 'value' && e.new === true);
    assert.ok(conv && typeof conv.trace_id === 'string', 'CONSUMER конвектора должен иметь trace_id');
    assert.equal(conv.trace_id, src.trace_id, 'CONSUMER конвектора обязан быть в trace_id источника DI/4');
